...
public class Poligono extends JFrame {
	...
	private int Pontos_n = 4;/* numero de vertice */
	...
	public void limpar_tela(Poligono p) {
		...
		g.clearRect(0, 0, p.getLarg(), p.getAlt());/** função resevada da bliblioteca que limpa a tela*/
	...
	public void desenhar() {
		...
		for (int i = 0; i < Pontos_n; i++) {
			...
			int y2 = Pontos_y[i];
			int x3 = Pontos_x[(i + 1)
					% Pontos_n];/* e necessario fazer a divisão para garantir que pecorremos todos os pontos */
	...
	public void pintar() {
		Graphics g = getGraphics();
		/* import java.awt.Polygon; */
		Polygon poly = new Polygon(Pontos_x, Pontos_y,
				Pontos_n);/* a criação desse objeto e necessaria para defirnimos o y min e o y max */
		...
		for (int y = minY; y < maxY; y++) {/* responsavel por pecorrer o y min ate o y max */
			...
			int x1 = Integer.MAX_VALUE;/*
										 * cria um interio de valor maximo possivel. Importante para definir o valor de
										 * x
										 */
			...
			boolean fora = false;/*
									 * booleano responsavel por verificar se a varredura esta fora ou dentro do
									 * poligono
									 */
